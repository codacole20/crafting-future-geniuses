  ...
  return (
    <div className="w-full bg-[#FAF8F3] rounded-[20px] shadow-ct py-4 px-5 flex justify-between items-center mb-3">
      <div>
        ...
        <div className="text-2xl font-bold text-[#7BB3E5] font-poppins">
          {typeof xp === 'number' ? xp : 0}
        </div>
      </div>
      <motion.div
        className="flex items-center"
        animate={streakGlow ? { scale: [1, 1.1, 1] } : undefined}
      ...
      >
        <div className="flex items-center bg-[#FACD7B] rounded-pill px-3 py-[0.3rem] min-w-[55px] justify-center shadow">
          ...
          <span className="font-poppins font-semibold text-gray-800">{typeof streak === 'number' ? streak : 0}</span>
        </div>
      </motion.div>
    </div>
  );
